...
class Flow extends  Component{
    ...
    queryFlowListSHow = async ()=>{
        let rows =   await API.queryFlowList();
        let billTypeList = [];
        let todoList = [];
        let billTypeArray = [];
    ...
    componentWillMount(){
        this.queryFlowListSHow();
    }

    sendHtml(data){
        sessionStorage.setItem('flow_titleName',data.busiName)
        this.props.history.push('/flowList/'+data.busiCode+'/'+data.billType);//跳转到代办详情页
    }
    returnPngs(billType) {
        switch (billType) {
            case  1: //转账支付
